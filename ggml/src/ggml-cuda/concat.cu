...
static __global__ void __launch_bounds__(CUDA_CONCAT_BLOCK_SIZE) concat_raw_cont(const T * x,
                                                                                 const T * y,
                                                                                 T *       dst,
                                                                                 int64_t   ne00,
                                                                                 int64_t   ne01,
...
template <typename T>
static void concat_raw_cont_cuda(const T * x,
                                 const T * y,
                                 T *       dst,
                                 int64_t   ne00,
                                 int64_t   ne01,
    ...
    if (dim == 0) {
        concat_raw_cont<T, 0><<<num_blocks, CUDA_CONCAT_BLOCK_SIZE, 0, stream>>>(x, y, dst, ne00, ne01, ne02, ne0, ne1, ne2);
        return;
    }
    if (dim == 1) {
        concat_raw_cont<T, 1><<<num_blocks, CUDA_CONCAT_BLOCK_SIZE, 0, stream>>>(x, y, dst, ne00, ne01, ne02, ne0, ne1, ne2);
        return;
    }
    concat_raw_cont<T, 2><<<num_blocks, CUDA_CONCAT_BLOCK_SIZE, 0, stream>>>(x, y, dst, ne00, ne01, ne02, ne0, ne1, ne2);
}
...
template <typename T>
static void concat_raw_cuda(const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst, int dim, cudaStream_t stream) {
    if (ggml_is_contiguous(src0) && ggml_is_contiguous(src1)) {
        ...
        if (dim != 3) {
            ...
            CUDA_CHECK(cudaMemcpyAsync(dst->data,                  src0->data, size0, cudaMemcpyDeviceToDevice, stream));
            CUDA_CHECK(cudaMemcpyAsync((char *) dst->data + size0, src1->data, size1, cudaMemcpyDeviceToDevice, stream));
        }
    } else {
        dim3 grid_dim(dst->ne[1], dst->ne[2], dst->ne[3]);
        auto launch_kernel = [&](auto dim_c) {
            concat_raw_non_cont<T, dim_c><<<grid_dim, CUDA_CONCAT_BLOCK_SIZE, 0, stream>>>(
...
void ggml_cuda_op_concat(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (src0->type != GGML_TYPE_F32) {
        ...
        switch (type_size) {
            case 1:
                concat_raw_cuda<uint8_t>(src0, src1, dst, dim, stream);
                return;
            case 2:
                concat_raw_cuda<uint16_t>(src0, src1, dst, dim, stream);
                return;
            case 4:
                concat_raw_cuda<uint32_t>(src0, src1, dst, dim, stream);
                return;
            default:
                GGML_ABORT("unsupported concat type size: %zu", type_size);
        }
    }
