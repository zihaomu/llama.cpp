...
static __device__ __forceinline__ float dsv4_e4m3fn_value(const int i) {
...
static __global__ void dsv4_fp8_kv_quantize_f32(
    ...
    for (int64_t off = 0; off < n_nope; off += 64) {
        const float v = *reinterpret_cast<const float *>(src_base + (off + tid)*nb00);
        scratch[tid] = fabsf(v);
        __syncthreads();
    ...
    for (int64_t i = n_nope + tid; i < ne00; i += 64) {
        *reinterpret_cast<float *>(dst_base + i*nb0) = *reinterpret_cast<const float *>(src_base + i*nb00);
    }
}
...
void ggml_cuda_op_dsv4_fp8_kv_quantize(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    const dim3 block_nums(n_rows, 1, 1);

    dsv4_fp8_kv_quantize_f32<<<block_nums, block_dims, 0, ctx.stream()>>>(
